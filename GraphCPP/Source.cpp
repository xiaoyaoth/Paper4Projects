...
int read_edge_list(int **tailp, int **headp, const char* filename)
{
	int max_edges = 140000;
	*tailp = (int *) calloc(max_edges, sizeof(int));
	*headp = (int *) calloc(max_edges, sizeof(int));
	...
	while ((ch = fgetc(fp)) != EOF) {
		...
		default:
			...
			if (isBlank)
				break;
			if (headTailToggle)
				(*headp)[nedges] = atoi(node);
			else
				(*tailp)[nedges++] = atoi(node);
			headTailToggle = !headTailToggle;
			isBlank = true;
			memset(node, 0, 10);
		}
	}
